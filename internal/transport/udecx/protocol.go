...
const (
	...
	// ControllerInterfaceGUID is the canonical device-interface class exposed
	// by the native controller. It is part of the ABI, not a discovery hint.
	ControllerInterfaceGUID = "{32d03f48-725b-4baa-970f-7f5de6c44687}"

	// Windows control-code inputs. Keep these platform-neutral so ABI contract
	// tests can run without a WDK or a Windows driver build.
	IOCTLBase         uint32 = 0x900
	fileDeviceUnknown uint32 = 0x22
	...
	IOCTLNegotiate           uint32 = fileDeviceUnknown<<16 | (fileReadData|fileWriteData)<<14 | (IOCTLBase+0)<<2 | methodBuffered
	IOCTLCreateDevice        uint32 = fileDeviceUnknown<<16 | (fileReadData|fileWriteData)<<14 | (IOCTLBase+1)<<2 | methodBuffered
	IOCTLDestroyDevice       uint32 = fileDeviceUnknown<<16 | (fileReadData|fileWriteData)<<14 | (IOCTLBase+2)<<2 | methodBuffered
	IOCTLDequeueOperation    uint32 = fileDeviceUnknown<<16 | (fileReadData|fileWriteData)<<14 | (IOCTLBase+3)<<2 | methodOutDirect
	IOCTLCompleteOperation   uint32 = fileDeviceUnknown<<16 | (fileReadData|fileWriteData)<<14 | (IOCTLBase+4)<<2 | methodInDirect
	IOCTLQueryStats          uint32 = fileDeviceUnknown<<16 | fileReadData<<14 | (IOCTLBase+5)<<2 | methodBuffered
	IOCTLSubmitInputReport   uint32 = fileDeviceUnknown<<16 | (fileReadData|fileWriteData)<<14 | (IOCTLBase+6)<<2 | methodInDirect
	IOCTLQueryLifecycleTrace uint32 = fileDeviceUnknown<<16 | fileReadData<<14 | (IOCTLBase+7)<<2 | methodBuffered
)
...
var (
	...
	ErrCapabilities      = errors.New("native UDE capabilities are unsupported")
...
func (m NegotiateRequest) MarshalBinary() ([]byte, error) {
	if m.ClientNonce == 0 {
		return nil, fmt.Errorf("%w: zero client nonce", ErrInvalidRange)
	}
	if m.RequestedCapabilities != AdvertisedCapabilities {
		return nil, fmt.Errorf(
			"%w: requested=%#x required=%#x",
			ErrCapabilities, m.RequestedCapabilities, AdvertisedCapabilities,
		)
	}
...
func ParseNegotiateResponse(src []byte) (NegotiateResponse, error) {
	...
	if h.Size != NegotiateResponseSize || len(src) != NegotiateResponseSize {
...
// ValidateNegotiation applies the fail-closed runtime compatibility contract
// after parsing a fixed-size response. ABI 1.14 requires an exact capability
// set and exact source-bound build identity; a merely compatible-looking
// driver is not accepted.
func ValidateNegotiation(response NegotiateResponse, clientNonce uint64, expectedBuildIdentity [BuildIdentitySize]byte) error {
	if clientNonce == 0 || response.ClientNonce != clientNonce || response.DriverNonce == 0 {
		return fmt.Errorf("%w: session nonce mismatch", ErrInvalidRange)
	}
	if response.Capabilities != AdvertisedCapabilities {
		return fmt.Errorf(
			"%w: driver=%#x required=%#x",
			ErrCapabilities, response.Capabilities, AdvertisedCapabilities,
		)
	}
	var zeroBuildIdentity [BuildIdentitySize]byte
	if subtle.ConstantTimeCompare(expectedBuildIdentity[:], zeroBuildIdentity[:]) == 1 {
		return fmt.Errorf("%w: expected build identity is unavailable", ErrBuildIdentity)
	}
	if subtle.ConstantTimeCompare(response.BuildIdentity[:], expectedBuildIdentity[:]) != 1 {
		return fmt.Errorf(
			"%w: driver=%s expected=%s",
			ErrIncompatibleABI,
			BuildIdentityHex(response.BuildIdentity),
			BuildIdentityHex(expectedBuildIdentity),
		)
	}
	if response.MaxDevices == 0 || response.MaxDescriptorBytes == 0 ||
		response.MaxTransferBytes == 0 || response.MaxIsoPackets == 0 ||
		response.MaxPendingOperations == 0 {
		return fmt.Errorf("%w: driver returned a zero negotiated limit", ErrInvalidRange)
	}
	if response.MaxDevices > MaxDevices || response.MaxDescriptorBytes > MaxDescriptorBytes ||
		response.MaxTransferBytes > MaxTransferBytes || response.MaxIsoPackets > MaxIsoPackets ||
		response.MaxPendingOperations > MaxPendingOperations {
		return fmt.Errorf("%w: driver limits exceed ABI bounds", ErrLimitExceeded)
	}
	return nil
}
...
// ParseDequeuedOperation binds the kernel's independent DeviceIoControl byte
// count to the embedded wire size before parsing one dequeued operation. The
// buffer may be larger than bytesReturned; only the returned prefix is message
// data.
...
func ParseStats(src []byte) (Stats, error) {
	...
	if h.Size != StatsSize || len(src) != StatsSize {
