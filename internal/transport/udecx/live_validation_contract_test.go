...
func TestNativeLiveReleaseGateRequiresCompleteEvidence(t *testing.T) {
	...
	for _, required := range []string{
		...
		"-ManageInstalledBrokerService is required",
		"$Iterations -lt 3",
		...
		"$ownedRootDevices[0].ConfigManagerErrorCode",
		"$infName -cnotmatch '^oem[0-9]+\\.inf$'",
		"$packageInfHash -cne $installedInfHash",
		"if ($SignatureValidationMode -ne 'LocalTest')",
		"$devnodes[0].IsSigned",
		"$devnodes[0].Signer -notmatch '(?i)Microsoft'",
		"Stop-Service -Name $brokerService.Name",
		"Start-Service -Name $brokerService.Name",
