...
// VIIPER modification: recover shell registration without losing initialization;
// keep retry timers and shutdown on the window owner. See README.viiper.md.
...
func (t *winTray) setIcon(src string) error {
	...
	if t.registration != nil && !t.registration.registered {
		return nil // Keep the icon for the pending NIM_ADD, not a doomed MODIFY.
	}
	return t.nid.modify()
}
...
func (t *winTray) wndProc(hWnd windows.Handle, message uint32, wParam, lParam uintptr) (lResult uintptr) {
	...
	case WM_CLOSE:
		t.stopShellRegistration()
		t.destroyNativeWindow()
	case WM_DESTROY:
		// same as WM_ENDSESSION, but throws 0 exit code after all
		defer func() { _, _, _ = pPostQuitMessage.Call(0) }() // Win32 void function.
		fallthrough
	case WM_ENDSESSION:
		t.stopShellRegistration()
		t.muNID.Lock()
		if t.nid != nil {
			if err := t.nid.delete(); err != nil {
				log.Printf("systray: unable to delete notification icon during shutdown: %v", err)
			}
		}
		t.muNID.Unlock()
		runSystrayExit()
	...
	case t.wmTaskbarCreated: // First Explorer startup or an Explorer restart.
		_ = t.registerShellIcon(true) // Registration owner logs failures.
	case WM_TIMER:
		if wParam == shellRetryTimerID {
			_ = t.registerShellIcon(false)
		}
	default:
...
func (t *winTray) initInstance() error {
	...
	if res == 0 {
		return fmt.Errorf("unable to register TaskbarCreated message: %v", err)
	}
	...
	_, _, _ = pShowWindow.Call(
		uintptr(t.window),
		uintptr(SW_HIDE),
	)

	_, _, _ = pUpdateWindow.Call(
		uintptr(t.window),
	)
	...
	t.registration = &shellRegistration{
		add:    t.nid.add,
		modify: t.nid.modify,
		armTimer: func() error {
			result, _, timerErr := pSetTimer.Call(uintptr(t.window), shellRetryTimerID,
				shellRetryMilliseconds, 0)
			if result == 0 {
				return fmt.Errorf("unable to schedule tray registration retry: %v", timerErr)
			}
			return nil
		},
		killTimer: func() {
			if result, _, err := pKillTimer.Call(uintptr(t.window), shellRetryTimerID); result == 0 {
				log.Printf("systray: unable to cancel registration timer: %v", err)
			}
		},
	}
	return nil
...
// Called only by registerSystray or the HWND message pump on its locked OS thread.
func (t *winTray) registerShellIcon(begin bool) error {
	t.muNID.Lock()
	defer t.muNID.Unlock()
	if t.registration == nil || !t.initialized.Load() || t.registration.stopped {
		return nil
	}
	var err error
	if begin {
		err = t.registration.begin()
	} else {
		err = t.registration.retry()
	}
	if err != nil && (t.registration.attempts == 1 || !t.registration.timerActive) {
		log.Printf("systray: shell registration pending (attempt %d/%d, retry timer=%t): %v",
			t.registration.attempts, maxShellRegistrationAttempts, t.registration.timerActive, err)
	}
	return err
}

func (t *winTray) stopShellRegistration() {
	t.muNID.Lock()
	defer t.muNID.Unlock()
...
func (t *winTray) destroyNativeWindow() {
	if result, _, err := pDestroyWindow.Call(uintptr(t.window)); result == 0 {
		log.Printf("systray: unable to destroy tray window: %v", err)
		return
	}
	if err := t.wcex.unregister(); err != nil {
		log.Printf("systray: unable to unregister tray window class: %v", err)
	}
}
...
func (t *winTray) showMenu() error {
	...
	_, _, _ = pSetForegroundWindow.Call(uintptr(t.window))
...
func iconToBitmap(hIcon windows.Handle) (windows.Handle, error) {
	...
	defer func() { _, _, _ = pReleaseDC.Call(0, hDC) }()
	hMemDC, _, err := pCreateCompatibleDC.Call(hDC)
	if hMemDC == 0 {
		return 0, err
	}
	defer func() { _, _, _ = pDeleteDC.Call(hMemDC) }()
	...
	defer func() { _, _, _ = pSelectObject.Call(hMemDC, hOriginalBmp) }()
...
func registerSystray() {
	if err := initializeTray(wt.initInstance, wt.createMenu, func() {
		wt.initialized.Store(true)
		systrayReady()
	}, func() error { return wt.registerShellIcon(true) }); err != nil {
		log.Printf("systray error: unable to initialize tray: %s\n", err)
		// Resource errors are different from Explorer being temporarily absent.
		// Retire any partial window on this same owner thread and let Run exit.
		if wt.window != 0 {
			wt.destroyNativeWindow()
		} else {
			runSystrayExit()
			_, _, _ = pPostQuitMessage.Call(0) // Win32 void function.
		}
	}
}
...
func quit() {
	...
	result, _, err := pPostMessage.Call(
		uintptr(wt.window),
		WM_CLOSE,
		0,
		0,
	...
	// WM_CLOSE/WM_DESTROY own timer cancellation, icon deletion and onExit.
	// Quit can be called from any goroutine; it must not retire native state here.
	if result == 0 {
		log.Printf("systray error: unable to post close request: %v", err)
	}
}
...
func removeMenuItem(item *MenuItem) {
	err := wt.removeMenuItem(item.id, item.parentId())
	if err != nil {
...
func resetMenu() {
	...
	if err := wt.createMenu(); err != nil {
		log.Printf("systray: unable to recreate menu: %v", err)
	}
}
