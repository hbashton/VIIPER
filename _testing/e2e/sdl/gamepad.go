...
// GamepadButtonEvent is one SDL transition from an exact opened gamepad.
// TimestampNS is SDL's monotonic event timestamp, not QPC.
...
// SDL requires the caller to own the event queue on its initialization thread;
// unrelated events are consumed while this method searches for the edge.
func (g *Gamepad) WaitButtonTransition(
	...
	return GamepadButtonEvent{
		Down: bool(down), TimestampNS: uint64(timestampNS),
	}, true, nil
}

// PollButtonTransition drains SDL's queued events and returns an exact edge
// for this gamepad/button without waiting. Unrelated events are consumed, so
// callers must give the observer exclusive ownership of SDL's event queue.
func (g *Gamepad) PollButtonTransition(
	button GamepadButton,
) (GamepadButtonEvent, bool, error) {
	if g == nil || g.cGamepad == nil {
		return GamepadButtonEvent{}, false, &SDLError{eStr: "invalid gamepad handle"}
...
// TicksNS returns SDL's monotonic clock used by gamepad event timestamps.
