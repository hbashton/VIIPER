...
static inline int wait_gamepad_button_event(
...
// WaitButtonEvent waits for an exact transition from this gamepad without
// polling SDL in a busy loop. SDL's event wait pumps the gamepad event queue
// and returns the transition generated by Windows, so benchmark CPU and
// scheduler-tail measurements are not contaminated by a synthetic observer
// consuming an entire core.
//
// SDL requires event waits to run on the thread which initialized the event
// subsystem. The e2e benchmark calls this method directly from its locked main
// test thread.
func (g *Gamepad) WaitButtonEvent(button GamepadButton, down bool, timeoutMS int32) bool {
	if g == nil || g.cGamepad == nil {
		return false
	}
	return C.wait_gamepad_button_event(
